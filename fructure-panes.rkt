...
(define (update-gui)
  (let* ([new-main-board (new fruct-board%)]
         [new-kit-board (new fruct-ed% [parent-editor "none"] [position '(0)])]
         [new-stage-board (new fruct-ed% [parent-editor "none"] [position '(0)])]
    ...
    (set! gui (build-gui-block source new-stage-board))
    (set! kit-gui (build-gui-block kit new-kit-board))
    #; (send new-stage-board set-snip! stage-board-snip)
    #; (send new-kit-board set-snip! kit-snip) 
...
(define fruct-ed%
  (class text% (super-new [line-spacing 0]) ; line spacing changes something.. padding?
...
; setup kit
(define original-kit '(env))
(define kit (simple-select original-kit))
...
(define main-board (new fruct-board%))
(define my-board-snip (new fruct-sn% [editor my-board] [parent-editor main-board] [position '()]))
#; (send my-board set-snip! my-board-snip)
(send main-board insert my-board-snip)
(send main-board move-to my-board-snip 200 0)

(define kit-board (new fruct-ed%
                       [parent-editor "none"]
                       [position '()]))
(define kit-snip (new fruct-sn% [editor kit-board] [parent-editor main-board] [position '()]))
#; (send kit-board set-snip! kit-snip)
(send main-board insert kit-snip)
...
(define kit-gui (build-gui-block kit kit-board))
...
; --------------------
(send my-canvas set-editor main-board)
;(send my-canvas set-editor my-board)
(send main-board set-caret-owner #f 'global)
(send my-frame show #t)
